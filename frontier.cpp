...
//NOTE will comment out all the cin and cout to eventually eliminate iostream
	...
	for (int j = 0; j < HEIGHT; ++j)	{
		for (int i = 0; i < WIDTH; ++i)	{
			if (ghostGrid[i][j] && blocks[i][j] == '.')	{
				...
				drawSingleBlock(blocks[i][j], i, j);
			}
		}
		/* NOTE cout interface related
		if (j == 1)	{
